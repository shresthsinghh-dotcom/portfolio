"""
integrated_analysis.py (debug-enabled)

Same behavior as before but returns an "error" string on failure.
This allows the Streamlit UI to show the full traceback and internal
state that caused the failure.
"""
...
# Force non-interactive backend for server use
matplotlib.use("Agg")
...
def _rgb_preview_from_tif(tif_file):
    """
    Attempt to open with rasterio and read expected bands.
    Returns (rgb_image, nir08, red, green, blue)
    """
    rewind(tif_file)
    with rio.open(tif_file) as tif_data:
        # Quick metadata for debugging
        meta = tif_data.meta.copy()
        # read band count
        bands = tif_data.count
        # Read required bands and cast to float
        nir08 = tif_data.read(1).astype(float)
        # If dataset has fewer bands than expected, raise helpful error
        if bands < 4:
            raise RuntimeError(f"Expected at least 4 bands for RGB preview; file has {bands} bands. meta: {meta}")
...
def run(tif_file, n_regions: int = 10, show_preview: bool = True) -> Dict[str, Any]:
    """
    Run integrated analysis. On exception, return a dict with 'error' key containing a full traceback.
    """
    try:
        # --- Build RGB preview and read bands ---
        rgb_image, nir08, red, green, blue = _rgb_preview_from_tif(tif_file)

        # --- Compute NDVI ---
        ndvi = compute_ndvi(red, nir08)

        # --- Compute temperature (REWIND REQUIRED) ---
        rewind(tif_file)
